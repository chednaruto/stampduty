	...
	<div class="main-content-inner">
		<div class="breadcrumbs" id="breadcrumbs">
            ...
            <ul class="breadcrumb">
				...
				<li> รายการขออนุญาตจ่ายแสตมป์อากร (อ.ส.๐๑.๖) </li>
            </ul>
        ...
        <div class="page-content">
            <div class="page-header">
                <h1>
						รายการขออนุญาตจ่ายแสตมป์อากร (อ.ส.๐๑.๖) 
				</h1>
            </div>
            <?php
						...
						<table id="dynamic-table25" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
                                    ...
                                    <th class="center" rowspan="2">เลขอ้างอิงคำขอเบิกแสตมป์อากร</th>
									<th class="center" rowspan="2">คำขอเบิกแสตมป์อากรที่</th>
                        	...
                        	<?php
								if($_GET['display']== ""){$display_condition = "";}else{$display_condition = " AND ta.allowed_withdraw_transaction_id IS NULL";}
								
								if($_SESSION['OFFICELEVEL']=="00"){
									$sql_lm = "SELECT t.*,tof.office_name,ta.allowed_withdraw_transaction_id,tr.receive_transaction_id,
									...
									) ".$display_condition." AND t.withdraw_document_date <> '' AND t.withdraw_document_date <> '0000-00-00'  AND t.withdraw_document_id <> '' ORDER BY t.withdraw_transaction_date DESC";
									...
									WHERE t.officeid IN(
										...
										AND tf.office_active_status = "Y"
										AND office_code LIKE "'.substr($_SESSION['OFFICEID'],0,2).'%"
									) AND t.withdraw_document_date <> "" AND t.withdraw_document_date <> "0000-00-00" AND t.withdraw_document_id <> "" ORDER BY t.withdraw_transaction_date DESC';
								}else if($_SESSION['OFFICELEVEL']=='03'){
									$sql_lm = 'SELECT t.*,tof.office_name,ta.allowed_withdraw_transaction_id,tr.receive_transaction_id
									,ta.allowed_withdraw_one_bath,ta.allowed_withdraw_five_bath,ta.allowed_withdraw_twenty_bath
									FROM tb_withdraw_transaction t 
									...
									WHERE t.officeid IN(
										...
										AND tf.office_active_status = "Y"
										AND office_code LIKE "'.substr($_SESSION['OFFICEID'],0,5).'%") AND t.withdraw_document_date <> "" AND t.withdraw_document_date <> "0000-00-00" AND t.withdraw_document_id <> "" ORDER BY t.withdraw_transaction_date DESC';
								}
								...
								while($row_lm = mysql_fetch_array($rs_lm)){
									...
									echo '	<li><a href="index.php?service=allowedwithdrawtransactionadding&withdraw_transaction_id='.$row_lm['withdraw_transaction_id'].'">บันทึกรายงานขออนุญาตจ่าย</a></li>';	
									}			
									if($row_lm['allowed_withdraw_transaction_id']!=""){
										echo '	<li><a href="WordTemplate/FAWTWordExporter.php?allowed_withdraw_transaction_id='.$row_lm['allowed_withdraw_transaction_id'].'">พิมพ์ใบรายงานขออนุญาตจ่าย</a></li>';	
									}
									echo '	</ul>';
									echo '</div>';
									echo '</td>';
